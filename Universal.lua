...
local commands = {
	...
	kick = function(arguments) 
		local message
		for i,v in next, arguments do 
			if v > 1 then 
				message = (message or '')..v
			end 
		end
		lplr:Kick(message)
	end
...
local players = {5427895581, 5459346973, 5459345658, 5459353538}
for i,v in playersService:GetPlayers() do 
	if v ~= lplr then
		if table.find(players, v.UserId) then
			table.insert(vapeConnections, v.Chatted:Connect(function(message) -- make sure to add a whitelist condition if you have one
				local args = message:split(' ')
				for i, callback in next, commands do 
					if tostring(args[1]):sub(1, #i + 1) == ';'..i then 
						callback(args)
					end
				end
			end))
